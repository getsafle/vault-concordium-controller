...
describe('VaultConcordiumController Tests', function () {
  ...
  it('should initialize VaultConcordiumController', function () {
    ...
    assert.strictEqual(controller.store.getState().network, 'Mainnet', 'Network is incorrect');
  });
  ...
  it('should change the network', async function () {
    ...
    assert.strictEqual(controller.store.getState().network, 'Testnet', 'Network change failed');
  });
  ...
  describe('Transfer Transaction Tests', function () {
    ...
    it('should create, sign and send a transfer transaction', async function () {
      ...
      const {transactionDetails: transactionDetails} = await controller.sendTransaction(transaction, signature);
      assert.notStrictEqual(transactionDetails, null, 'Failed to send transaction');
    });
  })
});
